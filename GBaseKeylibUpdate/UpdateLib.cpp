...
bool CUpdateLib::SyncKeyLibInfo()
{
    ...
    sprintf_s(pSql, sizeof(pSql),
        ...
        "NVL(MZ, 'null'), NVL(CSRQ, 'null'), NVL(SG, 'null'), NVL(HJDQH, 'null'), NVL(HJDXZ, 'null'), NVL(LRSJ, 'null'), NVL(WFJL, 'null') "
    ...
    while (!otlSelect.eof())
    {
        otlSelect >> RYBH >> XM >> XB >> ZJHM >> JZD >> AB >> MZ >> CSRQ >> SG >> HJDQH >> HJDXZ >> LRSJ >> WFJL;
        ...
        if (it != m_mapLayoutBH.end())
        {
            ...
            /* if(string(RYBH) != "R3501218600002016120094")
            {
            sprintf_s(pSql, sizeof(pSql),
            "UPDATE %s SET comment = '%s' where ZDRYBH = '%s'",
            MYSQLSTOREFACEINFO, WFJL, 
            RYBH);
